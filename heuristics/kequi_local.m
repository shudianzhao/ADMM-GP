function  [part_cell_final,part_final,newX_final,obj]= kequi_local(L,k,part_cell,part,X,obj);
...
% k: the partitioning number
    ...
    %  start from the current best results
            ...
            part_s = part_cell_new{pair_temp(1)};
            part_t = part_cell_new{pair_temp(2)};
            subX = Xnew_temp([part_s,part_t],[part_s,part_t]);
            
            subL = L([part_s,part_t],[part_s,part_t]);
            subL(1:(2*m+1):end)=0;
            subL(1:(2*m+1):end) = -sum(subL);
    ...
    %       apply 2opt
            ...
            [subx_new,new_xlx,old_xlx] = bisection_2opt(subL,subx);
            
            if new_xlx < old_xlx
                subX_simplex = subx_new'*subx_new;
                Xnew_temp([part_s,part_t],[part_s,part_t]) = (subX_simplex +1)/2;
                part_s_new = union(part_s(subx_new(1:m)==subx(1:m)),part_t(subx_new((m+1):end)~=subx((m+1):end)));
                part_t_new = union(part_s(subx_new(1:m)~=subx(1:m)),part_t(subx_new((m+1):end)==subx((m+1):end)));
...
function [xnew,cost,oldcost] = bisection_2opt(L,x);
% minimization
% obj: xLx
% {-1,1}
% equipartition
